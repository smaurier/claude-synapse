...
export function createLink(target, linkPath) {
    if (!isAbsolute(target)) {
        throw new Error(`synapse: le chemin cible doit être absolu, reçu "${target}". ` +
    ...
    try {
        ...
        throw new Error(`synapse: échec de création du lien "${linkPath}" -> "${target}".\n` +
            `Diagnostic : ${cause}\n` +
...
export function removeLink(linkPath) {
    ...
    if (!stat.isSymbolicLink()) {
        throw new Error(`synapse: refus de supprimer "${linkPath}" — ce n'est pas un lien (jonction/symlink), ` +
...
export function backupExisting(path) {
    ...
    if (stat.isSymbolicLink()) {
        throw new Error(`synapse: "${path}" est déjà un lien — backupExisting() est pour du contenu réel préexistant, ` +
 ...
 */
export function verifyWriteThrough(linkPath, hubPath) {
    ...
    try {
        ...
        if (existsSync(atHub))
            rmSync(atHub, { force: true });
        else if (existsSync(throughLink))
            rmSync(throughLink, { force: true });
    }
}
/**
 ...
 * purpose.
 */
export function ensureHubLink(hubClonePath, linkPath) {
...
//# sourceMappingURL=jonction.js.map
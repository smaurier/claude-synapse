 ...
 */
export function parseGithubOwnerRepo(hubUrl) {
    const stripGitSuffix = (s) => s.replace(/\.git$/, "");
    const ssh = hubUrl.match(/^git@github\.com:([^/]+)\/(.+)$/);
    if (ssh)
        return { owner: ssh[1], repo: stripGitSuffix(ssh[2]) };
    const https = hubUrl.match(/^https:\/\/github\.com\/([^/]+)\/([^/]+)$/);
    if (https)
        return { owner: https[1], repo: stripGitSuffix(https[2]) };
    return null;
}
export async function checkHubVisibility(hubUrl, fetchImpl = fetch) {
...
//# sourceMappingURL=hubVisibility.js.map
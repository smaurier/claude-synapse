...
function sessionResultsPath(pluginDataDir, sessionId) {
    return join(pluginDataDir, "session-results", `${sessionId}.json`);
}
export function writeSessionResults(pluginDataDir, sessionId, query, results) {
    const path = sessionResultsPath(pluginDataDir, sessionId);
    mkdirSync(join(pluginDataDir, "session-results"), { recursive: true });
    writeFileSync(path, JSON.stringify({ query, results }), "utf8");
}
export function readSessionResults(pluginDataDir, sessionId) {
    const path = sessionResultsPath(pluginDataDir, sessionId);
    if (!existsSync(path))
        return null;
 ...
 */
export function formatSessionResultsAsContext(stored) {
...
//# sourceMappingURL=sessionResults.js.map
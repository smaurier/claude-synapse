 ...
 */
const STRICT_MARKERS: RegExp[] = [/⛔/u, /\binterdit/iu];
const BROAD_MARKERS: RegExp[] = [
...
export function hasStrictNegationMarker(content: string): boolean {
...
export function hasBroadNegationMarker(content: string): boolean {
...
export function extractChunkText(sourcePath: string, content: string, chunkId: string | undefined): string {
  if (!chunkId) return content;
...
export function formatContradictionWarnings(results: HybridResult[], corpusByPath: Map<string, string>): string | null {
  const flagged = results.filter((r) => {
    const content = corpusByPath.get(r.path);
    if (content === undefined) return false;
    // STRICT: whole-file safety net, proven low-noise at that scope.
    if (hasStrictNegationMarker(content)) return true;
...
export async function runProactiveRecall(pluginDataDir: string, prompt: string): Promise<Record<string, string>> {
  ...
  const sections = [warnings, context].filter((s): s is string => s !== null);

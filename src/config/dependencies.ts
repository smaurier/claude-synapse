...
export function npmInstallProd(pluginRoot: string): void {
...
export function ensureDependencies(pluginRoot: string, install: (root: string) => void = npmInstallProd): void {
  if (existsSync(join(pluginRoot, "node_modules"))) return;
  install(pluginRoot);
}
